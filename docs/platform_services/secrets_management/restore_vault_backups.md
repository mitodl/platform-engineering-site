...
Threshold                0
Unseal Progress          0/0                   <<<<< This node knows nothing about any other nodes
Unseal Nonce             n/a
Version                  1.14.1
Build Date               2023-07-21T10:15:14Z
Storage Type             raft
HA Enabled               true                  <<<<< And it is supposed to!
```


## What to do!?!

1. Go to the appropriate S3 bucket and find the most recent backup. Production backs up every six hours. QA + CI backup once a day. Bucket names are: **ol-infra-ci-vault-backups**, **ol-infra-qa-vault-backups**, **ol-infra-production-vault-backups**.
...
9. Unset VAULT_TOKEN with `unset VAULT_TOKEN` (because that token was tied to the short-lived cluster that existed before running the restore). Then do a vault status: `vault status` and it should look something like this:
```
Key                      Value
---                      -----
...
11. Verify that the vault UI works as expected again and your secrets are there.
